 ...
 *
 * funkce doplní <!DOCTYPE>, část <head> a otevírací <body>
 *
...
/**
 * html_escape - zaescapování textu pro výpis do WWW stránky
 *
 * funkce nahradí znaky >, <, & a " 
 *
 * @param $text - řetězec, ve kterém se nahrazuje
 *
 * @return string - řetězec pro použití v HTML
 */
function html_escape($text)
{
  // náhrada "rizikových" HTML znaků za entity:
...
/**
 * escape_html - zaescapování textu pro výpis do WWW stránky
 *
 * funkce používá filtr pro escapování textu do HTML (porovnejte s funkcí "html_escape")
 *
 * @param $text - řetězec, který se escapuje
 *
 * @return string - řetězec pro použití v HTML
 */
...
/**
 * escape_url - zaescapování textu pro výpis do URL
 *
...
/**
 * presmeruj - přesměrovává na jinou stránku (volající skript nesmí NIC vypsat!!!)
 *
 * funkce přesměruje jinam (skript, který ji volal, nebude vůbec "vidět" v historii prohlížeče) a ukončí aktuální skript
 *
 * @param $kam - řetězec s názvem skriptu (relativní cesta vůči volajícímu), na nějž bude přesměrováno
 */
function presmeruj($kam)
{
  // zjistíme si, jaké je aktuální HTTP schéma, jaký je server a převezmeme adresář (URL) aktuálního skriptu:
  $odkud = $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['SERVER_NAME'].dirname($_SERVER['PHP_SELF']);
  header("Location: $odkud/$kam");
  exit; // konec skriptu
}