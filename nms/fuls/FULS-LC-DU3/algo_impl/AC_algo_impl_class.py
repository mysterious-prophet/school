...
class ACSearch(AutomatonSearch):
    ...
    def add_keyword(self, keyword, value):
        ...
        # Add the current node (last of the traversal for this specific keyword) to the list of output nodes
        self.AdjList[current_state]["output"].append((keyword,value))

    def set_fail_transitions(self):
        q = deque()
        child = 0
        # For each child of the root node to the queue and make their fail states be the root node 
        ...
        while q:
            ...
            for child in self.AdjList[r]["next_states"]:
                ...
                q.append(child)
                # Get the fail state of the popped node (= root node)
                ...
                # Set the child's output node (match is found) to be the index of the child + the index of the fail state node
    ...
    def pure_search(self, n, text, index_list):
        ...
        for i in range(0, n):
            # From the current state, find the characher. While the value is not found and the current state is not the root
