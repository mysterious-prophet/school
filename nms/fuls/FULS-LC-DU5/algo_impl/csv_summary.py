...
def get_header(num_runs, print_individual_runs, time_in_seconds):
	time_units = "[s]" if time_in_seconds == True else "[ms]"

	header = ["File size [MB]", "Algorithm", "Complexity"]
	if print_individual_runs == True:
		for i in range(0, num_runs):
			header.append("Run " + str(i+1) + " preprocess time " + time_units)
			header.append("Run " + str(i+1) + " search time " + time_units)
			header.append("Run " + str(i+1) + " total time " + time_units)
	header.append("Avg. preprocess time " + time_units)
	header.append("Avg. search time " + time_units)
	header.append("Avg. total time " + time_units)
	return header
...
def sort_times_into_cols(algorithms, print_individual_runs, time_in_seconds):
	for alg in algorithms:
		# Convert time units from ms to s
		if time_in_seconds == True:
			alg.preprocess_t_ms = [time/1000 for time in alg.preprocess_t_ms]
			alg.search_t_ms = [time/1000 for time in alg.search_t_ms]
...
def create_summary_csv(csv_name, file_size, algorithms, print_individual_runs, time_in_seconds):
	...
	header = get_header(num_runs, print_individual_runs, time_in_seconds)

	sort_times_into_cols(algorithms, print_individual_runs, time_in_seconds)
